tableextension 50010 EXCCRIVendor extends Vendor
{
    fields
    {
                ...
                if EXCCRIUserSetup.Get(UserId()) then begin
                    if Blocked <> Blocked::All then
                        if not EXCCRIUserSetup."Desbloquea Proveedores" then
                            ...
                            EXCCRIValidateCampaignRequirements.Maestros(Database::Vendor, "No.");
                            EXCCRIValidateCampaignRequirements.Dimensiones(Database::Vendor, "No.", 0, 0);
                        end;
                end else
                    Error(EXCCRIVendorUnlockPermissionErr);
            end;
        ...
        modify("VAT Registration No.")
        {
            ...
            trigger OnAfterValidate()
            var
                EXCCRIConsultaRNC: Codeunit 34003003;
            ...
            begin
                ...
                then
                    if EXCCRIRNCDGII.Get("VAT Registration No.") then begin
                        ...
                        EXCCRIConsultaRNC.BuscarRNC("VAT Registration No.", EXCCRIData);
