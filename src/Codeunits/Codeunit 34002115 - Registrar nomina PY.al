codeunit 34002115 "Registrar nomina PY"
{
    TableNo = 34002115;
            ...
            IF (Empleado."Fecha salida empresa" <> 0D) AND (Empleado."Fecha salida empresa" < PerInici) AND
               (Empleado."Calcular Nomina") THEN BEGIN
                IF Contrato."Fecha finalizacion" = 0D THEN BEGIN
                    ...
                    Contrato."Fecha finalizacion" := Empleado."Fecha salida empresa";
    ...
    begin
        CabNomina.RESET;
        CabNomina.SETRANGE(Periodo, GlobalRec."Inicio Periodo");
           ...
           (PerfilSal."Tipo Nomina" = GlobalRec."Tipo Nomina") THEN BEGIN
            ...
            IF PerfilSal."Currency Code" <> '' THEN
                IF ConfNominas."Tasa Cambio Calculo Divisa" <> 0 THEN BEGIN
                    ImporteTotal := ROUND(PerfilSal.Importe * ConfNominas."Tasa Cambio Calculo Divisa", ConfContab."Amount Rounding Precision");
                    ImporteBaseImp := ImporteTotal;
                END
                ELSE BEGIN
                    ...
                    ImporteTotal := ROUND(PerfilSal.Importe * CurrExchange."Relational Exch. Rate Amount",
                                          ConfContab."Amount Rounding Precision");
                    ImporteBaseImp := ImporteTotal;
                END;
    ...
    begin
        ...
        IF (PerfilSal."Tipo concepto" = PerfilSal."Tipo concepto"::Deducciones) AND (PerfilSal.Cantidad <> 0) AND
           (PerfilSal.Importe <> 0) AND (NOT PerfilSal."Sujeto Cotizacion") AND (PerfilSal."Tipo Nomina" = GlobalRec."Tipo Nomina") THEN BEGIN
            PerfilSal.Importe := PerfilSal.Cantidad * ROUND(PerfilSal.Importe, ConfContab."Amount Rounding Precision");
        ...
        IF DeduccGob.FINDSET(FALSE, FALSE) THEN
            REPEAT
                ...
                IF DeduccGob."Porciento Empleado" <> 0 THEN BEGIN
                    MontoAplicar := ROUND(ImporteCotizacion * DeduccGob."Porciento Empleado" / 100,
                                                      ConfContab."Amount Rounding Precision");
                ...
                */
                ...
                END;
                IF (LinAportesEmpresa.Importe <> 0) AND (NOT IndSkip) AND (NOT Empleado."Acumula Fondo Reserva") AND
                   (ConfNominas."Concepto AFP" = DeduccGob.Codigo) THEN BEGIN
                    LinTabla += 10;
                    PerfilSalTr.RESET;
                    PerfilSalTr.SETRANGE("No. empleado", Empleado."No.");
                    PerfilSalTr.SETRANGE("Concepto salarial", ConfNominas."Concepto INFOTEP");
                    PerfilSalTr.FINDFIRST;
                    "%Cot" := DeduccGob."Porciento Empresa";
                    PerfilSalTr.Importe := LinAportesEmpresa."Base Imponible";
                    PerfilSalTr.Cantidad := 1;
                    ImporteTotal := LinAportesEmpresa.Importe;
                    InsertNomina(PerfilSalTr);
                END
                ELSE
    ...
    begin
        ...
        */
        ...
        IF Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Quincenal THEN
            ...
            IngresoSalario := ROUND(IngresoSalario / 2, ConfContab."Amount Rounding Precision");
                ...
                IF LinEsqPercepISR."1ra Quincena" AND LinEsqPercepISR."2da Quincena" THEN BEGIN
                    ...
                    IF (HistLinNom.Total <> 0) AND (LinEsqPercepISR2."1ra Quincena" AND LinEsqPercepISR2."2da Quincena") THEN BEGIN
                        IF (HistLinNom."Tipo concepto" = HistLinNom."Tipo concepto"::Deducciones) AND (PrimeraQ) THEN
                            TotalISR[1] [1] += ROUND(HistLinNom.Total, ConfContab."Amount Rounding Precision")
                        ELSE
                            TotalISR[1] [1] += ROUND(HistLinNom.Total, ConfContab."Amount Rounding Precision");
        ...
        IF NOT RetencionISR.FINDSET(FALSE, FALSE) THEN
            EXIT;
        ...
        IF CantParam > 10 THEN
            CantParam := 10;

        FOR Indice := 1 TO CantParam DO BEGIN
            IF Indice < CantParam THEN BEGIN
                IF ((TotalISR[1] [1] * 12 >= RangoISR[Indice]) AND
                    (TotalISR[1] [1] * 12 < RangoISR[Indice + 1])) THEN BEGIN
                    Importe := (TotalISR[1] [1] * 12) - RangoISR[Indice];
                    Importe *= "%Calcular"[Indice] / 100;
                    Importe += ImporteRetencion[Indice];
                    Importe /= 12;
                    "%Cot" := "%Calcular"[Indice];
                    Indice := 10;
                END;
        ...
        TotalISR[1] [1] := ROUND(Importe, ConfContab."Amount Rounding Precision");
        ...
        IF (Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Quincenal) AND
           (PerfilSalImp."1ra Quincena") AND (PerfilSalImp."2da Quincena") AND (PrimeraQ) THEN
            TotalISR[1] [1] := ROUND(TotalISR[1] [1] / 2, ConfContab."Amount Rounding Precision")
        ELSE BEGIN
            HistLinNomISR.RESET;
        ...
        IF PerfilSalImp."% ISR Pago Empleado" <> 0 THEN BEGIN
            PerfilSalImp.Importe := ROUND(TotalISR[1] [1] * PerfilSalImp."% ISR Pago Empleado" / 100,
                                                            ConfContab."Amount Rounding Precision");
            ImporteTotal := ROUND(PerfilSalImp.Importe, ConfContab."Amount Rounding Precision") * -1;
            ...
            LinAportesEmpresa."% Cotizable" := ROUND("%Cot" * (100 - PerfilSalImp."% ISR Pago Empleado") / 100,
                                                            ConfContab."Amount Rounding Precision");
        ...
        IF PerfilSal.FINDSET(FALSE, FALSE) THEN
            REPEAT
                IF PerfilSal."Tipo concepto" = PerfilSal."Tipo concepto"::Deducciones THEN
                    ImporteTotal := (PerfilSal.Cantidad * ROUND(PerfilSal.Importe, ConfContab."Amount Rounding Precision")) * -1
                ELSE
                    ImporteTotal := PerfilSal.Cantidad * ROUND(PerfilSal.Importe, ConfContab."Amount Rounding Precision");
    ...
    begin
        ...
        InsertarDimTempDef(34002115);                                                       //Para las Dim del perfil de salario (linea del concepto salarial)
