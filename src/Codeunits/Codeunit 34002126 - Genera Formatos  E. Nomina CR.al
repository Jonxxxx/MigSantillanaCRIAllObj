codeunit 34002126 "Genera Formatos  E. Nomina CR"
{
    ...
    begin
        ...
        RepEmpresa.RESET;
        RepEmpresa.SETRANGE("Empresa cotizacion", CN."Empresa cotizacion");
        RepEmpresa.FINDFIRST;
        ...
        IF (UPPERCASE(BcoNom.Formato) = 'BCR') THEN BEGIN
            NombreArchivo := 'NOM-BCR-' + FuncNom.FechaCorta(FechaTrans) + '.txt';
        ...
        REPEAT
            ...
            IF UPPERCASE(BcoNom.Formato) = 'BCR' THEN BEGIN
                IF PrimeraVez THEN BEGIN
                    ...
                    IF STRLEN(RNC) < 12 THEN
                        RNC := PADSTR('', 12 - STRLEN(RNC), '0') + RNC;
                    Lin_Body += RNC;
                    Lin_Body += Secuencia;
                    Lin_Body += FORMAT(FechaTrans, 0, '<Day,2><Month,2><Year4>');
                    Lin_Body += '0004020' + Empresa."Identificador Empresa";
                    Lin_Body += FORMAT(Blanco, 12);
                    Lin_Body += '000000';
                    Lin_Body += FORMAT(Blanco, 158);
                    WriteExportLine(Lin_Body);

                    //Creo la linea de la cuenta de la empresa
                    CLEAR(Lin_Body);
                    Lin_Body := '000';
                    Lin_Body += BankAccount."Bank Account No.";
                    Lin_Body += '014'; //Moneda 01=Colones, 02=USD  + Codigo de transaccion(4)
                    Lin_Body += FORMAT(Blanco, 4);
                    Lin_Body += FORMAT(CERO, 8);
                    Lin_Body += FORMAT(TotalGeneral * 100, 12, '<integer,12><Filler Character,0>'); //Importe
                    Lin_Body += FORMAT(Blanco, 10); //Filler
                    Lin_Body += FORMAT(Blanco, 2); //Tipo de cedula

                    RepEmpresa."RNC/CED" := DELCHR(RepEmpresa."RNC/CED", '=', ' -');
                    Lin_Body += RepEmpresa."RNC/CED" + FORMAT(Blanco, 20 - STRLEN(RepEmpresa."RNC/CED"), '<Text>');

                    Concepto := STRSUBSTNO(Text002, GHCN.Inicio, GHCN.Fin);
                    Lin_Body += FORMAT(COPYSTR(Concepto, 1, 45), 45);
                ...
                Lin_Body += '012'; //Moneda 01=Colones, 02=USD  + Codigo de transaccion(2)
                Lin_Body += FORMAT(Blanco, 4);
                Lin_Body += FORMAT(CERO, 8);
                CLEAR(Total);
                GHLN.RESET;
                ...
                Lin_Body += FORMAT(Total * 100, 12, '<integer,12><Filler Character,0>'); //Importe
                Lin_Body += FORMAT(Blanco, 10); //Filler
                Lin_Body += FORMAT(Blanco, 2); //Tipo de cedula

                Empl."Document ID" := DELCHR(Empl."Document ID", '=', ' -');
                Lin_Body += Empl."Document ID" + FORMAT(Blanco, 20 - STRLEN(Empl."Document ID"), '<Text>');

                Concepto := Empl."Full Name";
                Lin_Body += FORMAT(COPYSTR(Concepto, 1, 45), 45);
                Lin_Body += FORMAT(Blanco, 35); //Filler
                IF Empl."Company E-Mail" <> '' THEN
                    Lin_Body += FORMAT(Empl."Company E-Mail", 50)
                ELSE
                    Lin_Body += FORMAT(Empl."E-Mail", 50);
        ...
        IF (UPPERCASE(BcoNom.Formato) = 'BLH') OR (UPPERCASE(BcoNom.Formato) = 'LOPEZDEHARO') THEN BEGIN
            ...
            WriteExportLine(Lin_Body2);
        END;
        DownloadTextExport(NombreArchivo2);
        ...
        */
        ...
        IF HSalario.FINDSET THEN
           REPEAT
              ...
              // TODO: Manual review - This entire salary-change export remains disabled and still depends on legacy server-path setup and obsolete employee-field assumptions.
        ...
        REPEAT
          ...
          IF Acumulado <> 0 THEN
             BEGIN
                ...
                // TODO: Manual review - This entire variable-salary export remains disabled and its source-field and activation requirements are not defined.
                // Original code: Archivo.WRITE(Lin_Body);
             END;
        ...
        REPEAT
          ...
          CASE Empl."Document Type" OF
           ...
           ELSE
            ...
            END;
          END;
          // TODO: Manual review - This entire journal-bank export remains disabled and requires an activation decision before its file flow can be migrated.
          // Original code: Archivo.WRITE(Lin_Body);
        UNTIL GenJnlLine.NEXT = 0;
        Window.CLOSE;
        Archivo.CLOSE;
    ...
    begin
        ...
        // TODO: Manual review - Bank Account field "E-Pay Export File Path" is unavailable; the disabled provider-export procedures also require missing payment metadata fields.
        ...
        CASE UPPERCASE(BankAccount.Formato) OF
            'BCR':
                FormatoBCR(CodDiario, SeccDiario);
            'BHD':
    ...
    begin
        WITH CheckLedgerEntry DO BEGIN
            ...
            */
            ...
            END;
            // TODO: Manual review - Check Ledger Entry fields "Trace No." and "Transmission File Name" and Gen. Journal Line field "Export File Name" are unavailable.
            // Original code preserved below.
            // "Trace No." := Trace;
    ...
    var
        // TODO: Manual review - This entire vendor-payment email body remains disabled and depends on removed custom setup and payment fields; activating it requires a business decision.
    ...
    begin
        ...
        REPEAT
          ...
          IF PrimeraVez THEN
            BEGIN
              ...
              // TODO: Manual review - The complete BCR provider-payment implementation is disabled and depends on removed payment metadata fields.
              // Original code: Archivo.WRITE(Lin_Body);
            END;
          ...
          IF (GenJnlLine."Account Type" = GenJnlLine."Account Type"::"Bank Account") AND  // Para cuando es transferencias entre bancos
             (GenJnlLine."Bal. Account Type" = GenJnlLine."Bal. Account Type"::"Bank Account") THEN
             BEGIN
              ...
              Lin_Body += '1';
              IF STRPOS(ConfContab."LCY Code",'US') <> 0 THEN
                BEGIN
                ...
                END
              ELSE
              IF STRPOS(ConfContab."LCY Code",'DO') <> 0 THEN
                BEGIN
        ...
        //          BcoACH.GET(VendorBank."Banco RED ACH");
                  IF (STRPOS(GenJnlLine."Currency Code",'DO') = 0) AND (BcoACH."Cod. Institucion Financiera" <> 'BPD') THEN
                     BEGIN
              ...
              IF STRPOS(ConfContab."LCY Code",'DO') <> 0 THEN
                 BEGIN
                  ...
                  BcoACH.GET(VendorBank."Banco RED ACH");
                  IF (GenJnlLine."Currency Code" = '') OR (BcoACH."Cod. Institucion Financiera" = 'BPD') THEN
                     BEGIN
                      Lin_Body += BcoACH."ACH Reservas";
                      Lin_Body += FORMAT(BcoACH."Digito Chequeo");
                     END
            ...
            // Original disabled BCR export write preserved below.
            // Original code: Archivo.WRITE(Lin_Body);
    ...
    begin
        ...
        REPEAT
          ...
          IF PrimeraVez THEN
            BEGIN
              ...
              // TODO: Manual review - The complete BHD provider-payment implementation is disabled and depends on removed payment metadata fields.
              // Original code: Archivo.WRITE(Lin_Body);
            END;
        ...
        //      BcoACH.TESTFIELD("Ruta y Transito");
          ...
          Lin_Body += Vendor."E-Mail";
            ...
            // Original disabled BHD export write preserved below.
            // Original code: Archivo.WRITE(Lin_Body);
    ...
    local procedure StartTextExport()
    begin
        Clear(TempBlob);
        TempBlob.CreateOutStream(StreamOut, TextEncoding::Windows);
    end;

    local procedure WriteExportLine(LineText: Text)
    begin
        StreamOut.WriteText(LineText);
        StreamOut.WriteText();
    end;

    local procedure DownloadTextExport(DownloadFileName: Text)
    begin
        TempBlob.CreateInStream(IStream, TextEncoding::Windows);
        DownloadFromStream(IStream, '', '', '', DownloadFileName);
    end;
}
