codeunit 34002114 "Registrar nomina HN"
{
    TableNo = 34002115;
        ...
        IF DeduccGob.FINDSET(FALSE, FALSE) THEN
            REPEAT
                ...
                */
                ...
                END;
                IF (LinAportesEmpresa.Importe <> 0) AND (NOT IndSkip) AND
                   (ConfNominas."Concepto AFP" = DeduccGob.Codigo) THEN BEGIN
                    ...
                    PerfilSalTr.Cantidad := 1;
                    ImporteTotal := LinAportesEmpresa.Importe * -1;
                    InsertNomina(PerfilSalTr);
                END
                ELSE
    ...
    begin
        ...
        IF Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Quincenal THEN
            IngresoSalario := ROUND(IngresoSalario / 2, 0.01);
        ...
        //MESSAGE('xx%1 %2 %3 %4',TotalISR[1][1],(RangoISR[1]/12));
        ...
        /*
        message('%1 %2 %3 %4 %5 %6',TotalISR[1][1], RangoISR[1]/12,Empleado."Cuota Colegio Anual",Empleado."Gastos Medicos",
                Empleado."Valor Donacion Anual", Empleado."Cuota Pension Anual",ConfNominas."Importe Anual IHSS Base ISR");
        */
        Base := TotalISR[1] [1] / 12;
        FOR Indice := 1 TO CantParam DO BEGIN
            IF Indice < CantParam THEN BEGIN
                IF ((TotalISR[1] [1] >= RangoISR[Indice]) AND
                    (TotalISR[1] [1] < RangoISR[Indice + 1])) THEN BEGIN
                    Importe := (TotalISR[1] [1]) - RangoISR[Indice];
            ...
            END
            ELSE BEGIN
                //       MESSAGE('%1 %2 %3 %4 %5',RangoISR[Indice],Indice,"%Calcular"[Indice],ImporteRetencion[Indice],cantparam);
                Importe := (TotalISR[1] [1]) - RangoISR[Indice];
                Importe *= "%Calcular"[Indice] / 100;
                Importe += ImporteRetencion[Indice];
                Importe /= 12;
        ...
        TotalISR[1] [1] := ROUND(Importe, 0.01);
