codeunit 34002160 "Registrar nomina CR New"
{
    ...
    begin
        ...
        IF Empleado."Tipo pago" = Empleado."Tipo pago"::"Sueldo fijo" THEN BEGIN
            EsqSal.RESET;
        ...
        IF ((PerfilSal."Tipo concepto" = PerfilSal."Tipo concepto"::Ingresos) AND
           (PerfilSal."Tipo de nomina" = GlobalRec."Tipo de nomina")) OR (TiposNom."Incluir salario" AND PerfilSal."Salario Base") THEN BEGIN
            ...
            IF (PerfilSal."Salario Base") AND (ConfNominas."Concepto Sal. Base" = PerfilSal."Concepto salarial") THEN BEGIN
                    ...
                    IF Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Mensual THEN BEGIN
                        ...
                        IF Empleado."Employment Date" > PerInici THEN BEGIN
                            IF (Empleado."Termination Date" <= PerFinal) AND (Empleado."Termination Date" <> 0D) THEN
                                CantidadDiasEnt := Empleado."Termination Date" - Empleado."Employment Date" + 1
                            ELSE
                                IF (DATE2DMY(PerFinal, 1) > 30) OR (DATE2DMY(PerFinal, 2) = 2) THEN
                                    CantidadDiasEnt := 30 - DATE2DMY(Empleado."Employment Date", 1) + 1
                                ELSE
                                    CantidadDiasEnt := PerFinal - Empleado."Employment Date" + 1;
                            //ERROR('%1 %2 %3 %4 %5',CantidadDiasEnt,Empleado."Termination Date",PerFinal);
                            CantidadDiasEnt := CantidadDiasEnt - DiasAusencia;
                ...
                /*
             IF Empleado."Fin contrato" <> 0D THEN
                IF (Empleado."Fin contrato" >= PerInici) AND (Empleado."Fin contrato" <= PerFinal) THEN
                   BEGIN
                   ...
                   //Para no incluir en el calculo los dias con incidencias
                    ConfNominas.TESTFIELD("Metodo calculo Ingresos");
                    ParamCalcDias.GET(ConfNominas."Metodo calculo Ingresos");
                    IF ParamCalcDias.Valor <> 30 THEN
                       BEGIN
                        Fecha.RESET;
                        Incidencias.RESET;
                        Incidencias.SETRANGE("Employee No.",Empleado."No.");
                  ...
                  //error('bb %1 %2 %3 %4',cantidaddiassal,diaspago,DiasIncid,calendar.COUNT,fecha.GETFILTERS);
                    IF Puestos."Incluye Dias Feriados" THEN
                       BEGIN
                        Calendar.RESET;
                ...
                IF DiasPago <> 0 THEN BEGIN
                    ...
                    IF (Empleado."Termination Date" >= PerInici) AND (Empleado."Termination Date" <= PerFinal) THEN BEGIN
                        ...
                        ImporteTotal := IngresoSalario / CalcDias.Valor * CantidadDiasSal;
                        ImporteBaseImp := ImporteTotal;
                    END;
                END;
            END;
    ...
    begin
        IF Empleado."Excluir Calc. Imp. en Comision" AND (TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Comisiones) THEN
            EXIT;
        DeduccGob.RESET;
        ...
        REPEAT
            ...
            IF Empleado."Excluido Cotizacion TSS" THEN BEGIN
                IF (DeduccGob.Codigo = ConfNominas."Concepto AFP") OR (DeduccGob.Codigo = ConfNominas."Concepto SFS") OR
                   (DeduccGob.Codigo = ConfNominas."Concepto SRL") THEN
                    IndSkip := TRUE;
            END;
            IF (NOT TiposNom."Cotiza AFP") AND (DeduccGob.Codigo = ConfNominas."Concepto AFP") THEN
                IndSkip := TRUE
            ELSE
                IF (NOT TiposNom."Cotiza SFS") AND (DeduccGob.Codigo = ConfNominas."Concepto SFS") THEN
                    IndSkip := TRUE
                ELSE
                    IF (NOT TiposNom."Cotiza INFOTEP") AND (DeduccGob.Codigo = ConfNominas."Concepto INFOTEP") THEN
                        IndSkip := TRUE
                    ELSE
                        IF (NOT TiposNom."Cotiza SRL") AND (DeduccGob.Codigo = ConfNominas."Concepto SRL") THEN
                            IndSkip := TRUE;
                    ...
                    REPEAT
                        IF SegundaQ THEN BEGIN
                            ...
                            IF NOT HistCabNom.FINDFIRST THEN
                                HistCabNom.INIT
                            ELSE
                                IF DATE2DMY(LinNominasES.Periodo, 1) <> TiposNom."Dia inicio 1ra" THEN
                    ...
                    IF Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Quincenal THEN BEGIN
                        IF (PerfilSalTr."1ra Quincena") AND (NOT PerfilSalTr."2da Quincena") AND (PrimeraQ) THEN BEGIN
                            IF TiposNom."Dia inicio 1ra" > TiposNom."Dia inicio 2da" THEN
                                LinNominasES.SETRANGE(Periodo, DMY2DATE(TiposNom."Dia inicio 1ra", DATE2DMY(CALCDATE('-1M', PerInici), 2), DATE2DMY(CALCDATE('-1M', PerInici), 3)), PerFinal)
                            ELSE
                                IF TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Comisiones THEN
                                    LinNominasES.SETRANGE(Periodo, DMY2DATE(TiposNom."Dia inicio 1ra", DATE2DMY(PerInici, 2), DATE2DMY(PerInici, 3)), PerFinal)
                                ELSE
                                    LinNominasES.SETRANGE(Periodo, PerInici, PerFinal);
                            IF ConfNominas."Concepto AFP" = DeduccGob.Codigo THEN
                                LinNominasES.SETRANGE("Cotiza AFP", TRUE)
                                    ...
                                    IF PerfilSalTr."1ra Quincena" AND PerfilSalTr."2da Quincena" THEN BEGIN
                                        ...
                                        ELSE
                                            IF TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Comisiones THEN
                                                LinNominasES.SETRANGE(Periodo, DMY2DATE(TiposNom."Dia inicio 1ra", DATE2DMY(PerInici, 2), DATE2DMY(PerInici, 3)), PerFinal)
                                            ELSE
                                                IF (SegundaQ) AND (TiposNom."Frecuencia de pago" <> TiposNom."Frecuencia de pago"::Diaria) THEN
                                                    LinNominasES.SETRANGE(Periodo, DMY2DATE(TiposNom."Dia inicio 1ra", DATE2DMY(PerInici, 2), DATE2DMY(PerInici, 3)), PerFinal)
                                                ELSE
                                                    LinNominasES.SETRANGE(Periodo, PerInici, PerFinal);
            ...
            IF (NOT SegundaQ) AND (ConfNominas."Metodo Calculo SS" <> ConfNominas."Metodo Calculo SS"::"Ingresos del Periodo") THEN BEGIN
                TN.RESET;
            ...
            IF NOT IndSkip THEN BEGIN
                ...
                IF ((Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Quincenal) AND
                   ...
                   (SegundaQ)) THEN BEGIN
                    ...
                    ELSE
                        IF TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Comisiones THEN
                            LinAportesEmpresa2.SETRANGE(Periodo, DMY2DATE(TiposNom."Dia inicio 1ra", DATE2DMY(PerInici, 2), DATE2DMY(PerInici, 3)), PerFinal)
                        ELSE
                            IF (SegundaQ) AND (TiposNom."Frecuencia de pago" <> TiposNom."Frecuencia de pago"::Diaria) THEN
                                LinAportesEmpresa2.SETRANGE(Periodo, DMY2DATE(TiposNom."Dia inicio 1ra", DATE2DMY(PerInici, 2), DATE2DMY(PerInici, 3)), PerFinal)
                            ELSE
                    ...
                    LinAportesEmpresa.Importe += ROUND(ABS(ImporteImpuestosemp), 0.01);
                END;
    ...
    begin
        ...
        IF (Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Quincenal) AND (TiposNom."Tipo de nomina" <> TiposNom."Tipo de nomina"::Bonificacion) THEN BEGIN
        ...
        IF SegundaQ OR (TiposNom."Frecuencia de pago" = TiposNom."Frecuencia de pago"::Mensual) THEN // Cuando se descuenta solo en la segunda
           BEGIN
            HistLinNom.RESET;
            HistLinNom.SETRANGE("No. empleado", GlobalRec."No. empleado");
            IF (TiposNom."Dia inicio 1ra" > TiposNom."Dia inicio 2da") AND (SegundaQ) AND (TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Regular) AND
               (TiposNom."Dia inicio 2da" <> 0) THEN
        ...
        IF NOT SegundaQ THEN BEGIN
            IF (TiposNom."Dia inicio 1ra" > TiposNom."Dia inicio 2da") AND (SegundaQ) AND (TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Regular) THEN
                HistLinNom.SETRANGE(Periodo, DMY2DATE(TiposNom."Dia inicio 1ra", DATE2DMY(CALCDATE('-1M', PerInici), 2), DATE2DMY(CALCDATE('-1M', PerInici), 3)), PerFinal)
            ELSE
                IF (TN."Dia inicio 1ra" > TN."Dia inicio 2da") AND ((TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Prestaciones) OR (TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Bonificacion)) THEN BEGIN
                    HistLinNom.SETRANGE(Periodo, DMY2DATE(TN."Dia inicio 1ra", DATE2DMY(CALCDATE('-1M', PerInici), 2), DATE2DMY(CALCDATE('-1M', PerInici), 3)), PerFinal);
                ...
                ELSE
                    ...
                    END
                    ELSE
                        IF (TN."Dia inicio 1ra" > TN."Dia inicio 2da") AND (TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Extra) THEN
                            HistLinNom.SETRANGE(Periodo, DMY2DATE(TN."Dia inicio 1ra", DATE2DMY(CALCDATE('-1M', PerInici), 2), DATE2DMY(CALCDATE('-1M', PerInici), 3)), PerFinal)
                        ELSE
                            HistLinNom.SETRANGE(Periodo, PerInici, PerFinal);
        END
        ...
        IF HistLinNom.FINDSET(FALSE, FALSE) THEN
            REPEAT
                ...
                IF (LinEsqPercepISR."1ra Quincena" AND LinEsqPercepISR."2da Quincena") AND
                  (Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Quincenal) THEN BEGIN
                    IF ((Puestos."Metodo Calculo Paga Salario" = 0) AND (HistLinNom."Salario Base" AND PrimeraQ) AND (PrimeraVez) AND
                       (TiposNom."Frecuencia de pago" = TiposNom."Frecuencia de pago"::Quincenal)) OR (TiposNom."Calcular ISR Mes en Bonific") THEN BEGIN
        ...
        IF TiposNom."Tipo de nomina" <> TiposNom."Tipo de nomina"::Bonificacion THEN BEGIN
            HistLinNom.RESET;
            HistLinNom.SETRANGE("No. empleado", GlobalRec."No. empleado");
            IF (TiposNom."Dia inicio 1ra" > TiposNom."Dia inicio 2da") AND (SegundaQ) AND ((TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Regular) OR
               (TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Comisiones)) THEN
                HistLinNom.SETRANGE(Periodo, DMY2DATE(TiposNom."Dia inicio 1ra", DATE2DMY(CALCDATE('-1M', PerInici), 2), DATE2DMY(CALCDATE('-1M', PerInici), 3)), PerFinal)
            ELSE
                IF (TN."Dia inicio 1ra" > TN."Dia inicio 2da") AND ((TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Prestaciones) OR (TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Bonificacion)) THEN
                    HistLinNom.SETRANGE(Periodo, DMY2DATE(TN."Dia inicio 1ra", DATE2DMY(CALCDATE('-1M', PerInici), 2), DATE2DMY(CALCDATE('-1M', PerInici), 3)), PerFinal)
            ...
            IF ConfNominas."Metodo Calculo SS" <> ConfNominas."Metodo Calculo SS"::"Ingresos del Periodo" THEN BEGIN
                ...
                ELSE
                    IF (TN."Dia inicio 1ra" > TN."Dia inicio 2da") AND ((TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Prestaciones) OR (TiposNom."Tipo de nomina" = TiposNom."Tipo de nomina"::Bonificacion)) THEN
                        HistLinNom.SETRANGE(Periodo, DMY2DATE(TN."Dia inicio 1ra", DATE2DMY(CALCDATE('-1M', PerInici), 2), DATE2DMY(CALCDATE('-1M', PerInici), 3)), PerFinal)
                    ELSE
                        IF (SegundaQ) AND (TN."Dia inicio 1ra" > TN."Dia inicio 2da") THEN
                            HistLinNom.SETRANGE(Periodo, DMY2DATE(TN."Dia inicio 1ra", DATE2DMY(CALCDATE('-1M', PerInici), 2), DATE2DMY(CALCDATE('-1M', PerInici), 3)), PerFinal)
                        ELSE
                            IF SegundaQ THEN
                                HistLinNom.SETRANGE(Periodo, DMY2DATE(TN."Dia inicio 1ra", DATE2DMY(PerInici, 2), DATE2DMY(PerInici, 3)), PerFinal)
                ...
                ELSE
                    IF PrimeraQ THEN BEGIN
                        ...
                        LinEsqPercepISR2.SETRANGE("Cotiza ISR", TRUE);
                        // LinEsqPercepISR2.SETFILTER(Cantidad,'>=%1',1);
                        LinEsqPercepISR2.SETRANGE("Tipo de nomina", TiposNom.Codigo);
        ...
        */
        ...
        IF ((Contrato."Frecuencia de pago" = Contrato."Frecuencia de pago"::Quincenal) AND
           ...
           (Puestos."Metodo Calculo Paga Salario" = 0)) AND (TiposNom."Frecuencia de pago" <> TiposNom."Frecuencia de pago"::Mensual) THEN
        ...
        IF SaldoFavor."Importe Pendiente" <> 0 THEN BEGIN
            ...
            PerfilSalImp.Comentario := STRSUBSTNO(Text002, PerfilSalImp.Descripcion, FORMAT(ROUND(TotalISR[1] [1], 0.01), 0, '<Integer thousand><Decimals,3>'), FORMAT(ROUND(SaldoFavor."Importe Pendiente" + ISRCompensado, 0.01), 0, '<Integer thousand><Decimals,3>'));
        ...
        IF (ISRCobrado >= TotalISR[1] [1]) AND (TotalISR[1] [1] <> 0) AND (TiposNom."Tipo de nomina" <> TiposNom."Tipo de nomina"::Prestaciones) AND (NOT SaldoCompensado) THEN
            EXIT
        ELSE
            IF (TiposNom."Tipo de nomina" <> TiposNom."Tipo de nomina"::Prestaciones) AND (TotalISR[1] [1] <> 0) THEN BEGIN
    ...
    begin
        ...
        //FuncionesNom.CalculoDiaVacacionesCR
        ...
        //     EXIT;
        // Nueva regla es 1 dia por cada mes y al cumplir cada año se suman 10 dias por Santillana
        Contrato.RESET;
        Contrato.SETRANGE("No. empleado", Empleado."No.");
        Contrato.SETRANGE("Cod. contrato", Empleado."Emplymt. Contract Code");
        Contrato.SETRANGE(Activo, TRUE);
        Contrato.FINDFIRST;
        ...
        IF Empleado."Dias Vacaciones" > 0 THEN
          BEGIN
            HistVac.RESET;
            HistVac.SETRANGE("No. empleado",Empleado."No.");
            HistVac.SETRANGE("Tipo calculo",HistVac."Tipo calculo"::Adicional);
            IF (DATE2DMY(Empleado."Employment Date",1) = 29) AND (DATE2DMY(Empleado."Employment Date",2) = 2) AND (DATE2DMY(Empleado."Employment Date",3) <> DATE2DMY(PerInici,3)) THEN
               BEGIN
                Fecha.RESET;
        ...
        */
        ...
        //MESSAGE('%1',DiasVac);
        IF (DiasVac <> 0) THEN //AND (DATE2DMY(Empleado."Employment Date",2) = DATE2DMY(PerInici,2)) THEN
           BEGIN
            FechaVacFin := DMY2DATE(DATE2DMY(Empleado."Employment Date", 1), DATE2DMY(Empleado."Employment Date", 2), DATE2DMY(CALCDATE('+1a', PerInici), 3));
            HistVac.RESET;
            HistVac.SETRANGE("No. empleado", Empleado."No.");
            HistVac.SETRANGE("Fecha Inicio", DMY2DATE(DATE2DMY(Empleado."Employment Date", 1), DATE2DMY(Empleado."Employment Date", 2), DATE2DMY(PerInici, 3)), FechaVacFin);
            ...
            IF Parametrosvacaciones.FINDSET THEN BEGIN
                ...
                //MESSAGE('%1 %2 %3 %4 %5',AnosEmpresa,RangoAnos[1],RangoAnos[2],RangoAnos[3]);
                HistVac.RESET;
                HistVac.SETRANGE("No. empleado", Empleado."No.");
                HistVac.SETRANGE("Fecha Inicio", DMY2DATE(DATE2DMY(Empleado."Employment Date", 1), DATE2DMY(Empleado."Employment Date", 2), DATE2DMY(PerInici, 3)), FechaVacFin);
                HistVac.SETRANGE("Tipo calculo", HistVac."Tipo calculo"::Adicional);
                IF HistVac.FINDFIRST THEN;
                    ...
                    ELSE
                        ...
                        ELSE
                            ...
                            ELSE
                                ...
                                END
                                ELSE
                                    IF (AnosEmpresa >= RangoAnos[1]) AND (RangoAnos[1] > 0) THEN BEGIN
                                        HistVac.INIT;
                                        HistVac."No. empleado" := Empleado."No.";
                                        HistVac."Fecha Inicio" := DMY2DATE(DATE2DMY(Empleado."Employment Date", 1), DATE2DMY(Empleado."Employment Date", 2), DATE2DMY(PerInici, 3));
                                        HistVac."Fecha Fin" := CALCDATE('+1A', HistVac."Fecha Inicio");
    ...
    begin
        ...
        ELSE
            LinNomina.SETRANGE(Periodo, DMY2DATE(1, 1, DATE2DMY(PerInici, 3)), PerFinal);
    ...
    begin
        ...
        REPEAT
            ...
            IF ((DATE2DMY(PerInici, 1) <> 1) AND (DATE2DMY(PerInici, 2) = 1)) AND (TiposNomPrest."Dia inicio 1ra" <> 1) THEN
            ...
            ELSE
                IF (DATE2DMY(PerInici, 1) <> 1) AND (DATE2DMY(PerFinal, 2) <> 1) AND (TiposNomPrest."Dia inicio 1ra" > TiposNomPrest."Dia inicio 2da") THEN
                ...
                ELSE
                    LinNominasES.SETRANGE("Fin periodo", DMY2DATE(1, DATE2DMY(PerInici, 2), DATE2DMY(PerInici, 3)), PerFinal);
    ...
    var
        DimensionesContab: Record 55773;
