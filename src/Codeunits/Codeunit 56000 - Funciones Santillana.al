codeunit 56000 "Funciones Santillana"
{
    ...
    //                              e Importe.
    // #4161   29/09/2014    PLB    Sólo permitir registrar la hoja de ruta si las líneas de pedido venta tienen rellenado
    //                              el no. factura
    // 
    ...
    // 003     21/10/2025    LDP    SANTINAV-8697: Problema con código QR en facturas electrónicas ã Costa Rica
    ...
    begin
        SELECTLATESTVERSION;
        REPORT.RUN(REPORT::Report51003, FALSE, FALSE);
    end;
    ...
    begin
        ...
        IF NOT CONFIRM('Periodo %1 - %2. ¿Cambiar fecha inicio a "%3"?', FALSE, Contrato."Fecha inicio", Contrato."Fecha finalización", NuevaFechaInicio) THEN BEGIN
            Contrato."Fecha inicio" := 011816D;
            IF (Contrato."Fecha finalización" <> 0D) AND (Contrato."Fecha inicio" > Contrato."Fecha finalización") THEN
                ERROR('La fecha inicio no puede ser superior a la fecha finalización');
            Contrato.MODIFY;
            MESSAGE('Nuevo periodo %1 - %2', Contrato."Fecha inicio", Contrato."Fecha finalización");
        END;
    end;
