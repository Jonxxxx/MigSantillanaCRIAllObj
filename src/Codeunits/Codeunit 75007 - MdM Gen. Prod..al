codeunit 75007 "MdM Gen. Prod."
{
                    ...
                    IF rImp."Id Tabla" = 27 THEN BEGIN
                        ...
                        IF rField.FINDSET THEN BEGIN
                            ...
                            REPEAT
                                ...
                                CASE rField."Id Field" OF
                                    ...
                                    // -399..-350 Lo reservasmos para precios de compra si algún dia hace falta
                                    // -501 Lo reservamos para codigo de divisa
                                    // -901 Lo reservamos para guardar la Codigo de Articulo Pack Anterior en modificaciones
                                    // cod. barras
                                    // 25/10/2018 Se solicita que la descripcion del codigo de barras sea la del producto
                                    -499 .. -400:
                            ...
                            // en el update, si alguna dimension actualizada es global, hay que actualizar el producto
                        ...
                        IF rImp.Operacion IN [rImp.Operacion::Insert, rImp.Operacion::Update] THEN BEGIN
                            ...
                            IF ConfiguraTipologiaMdM(Item, rImp) THEN
                                lwMod := TRUE;
                        END;
                        // Solo en el insert de la importacion por Web Service
                        IF (prCab.Operacion IN [prCab.Operacion::Insert]) AND (prCab.Entrada = prCab.Entrada::INT_WS) THEN BEGIN
                            // MdM Hay una configuracion a partir de estructura analitica
                            IF cGestMdM.SetEstrAnalitica(Item) THEN
                                lwMod := TRUE;

                            // Hay una modificacion por campos relacionados
    ...
    begin
        ...
        END
        ELSE BEGIN
            // Buscamos el útlimo Nº de linea
            CLEAR(lwLinNo);
            CLEAR(lrLinBOM);
            lrLinBOM.SETRANGE("Parent Item No.", pwCod);
        ...
        ELSE BEGIN
            IF rCab.Entrada = rCab.Entrada::INT_WS THEN BEGIN
                IF prImp."Id Tabla" = 27 THEN BEGIN
                    ...
                    // Buscamos en la configuracion de tipologia
            ...
            // Esto es despues de la inserccion
    ...
    begin
        ...
        IF NOT lwEnc THEN BEGIN
            // Buscamos el último no de linea
            CLEAR(lrComntLine);
            lrComntLine.SETRANGE("Table Name", lrComntLine."Table Name"::Item);
            lrComntLine.SETRANGE("No.", pwCodPro);
    ...
    begin
        // FindTipoConf
        // Buscamos la serie Configuracion Tipologia
        ...
        lwTip := FindFieldValue(lwId1, lwId2, 5702); // Busca la tipologia en la importacion
        IF lwTip = '' THEN
            EXIT;
