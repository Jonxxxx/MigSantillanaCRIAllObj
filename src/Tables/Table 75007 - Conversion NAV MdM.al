table 75007 "Conversion NAV MdM"
{
    ...
    fields
    {
            ...
            begin
                ...
                IF lwLng > 0 THEN BEGIN
                    IF "Tipo Registro" = "Tipo Registro"::ISBN THEN BEGIN
                        ...
                        //TODO Ver: rConfMdM.GET;
                        //TODO Ver: IF rConfMdM."Control ISBN" THEN
                        //TODO Ver:     cFunMdM.ControlIBN("Codigo NAV", TRUE);
                    END;
                END;

                // Controlamos la longitud m xima
                //TODO Ver: lwMaxLng := GetMaxLen;
                lwLng := STRLEN("Codigo NAV");
        ...
        field(3; "Codigo NAV"; Code[20])
        {
            ...
            ELSE IF ("Tipo registro" = CONST("Pais")) Country/Region.Code
                            ELSE IF ("Tipo registro"=CONST("Edicion")) Edicion.Codigo
                            ELSE IF ("Tipo registro"=CONST(Estado)) "Estado productos".Codigo
                            ...
                            ELSE IF ("Tipo registro"=CONST("Campana")) "Datos MDM".Codigo WHERE ("Tipo"=CONST("Campana"))
    ...
    begin
        ...
        CASE pwId OF
            0:
                wTipo := "Tipo Registro"::"Serie/M todo";
            1:
    ...
    begin
        ...
        lwIdDim := GetDimId;
        IF lwIdDim > -1 THEN BEGIN
            //TODO Ver: lwDimCode := cFunMdM.GetDimCode(lwIdDim, TRUE);
            CLEAR(lrDimVal);
            ...
            lwOK := lrDimVal.GET(lwDimCode, "Codigo MdM"); // Posicionamos el valor por defecto
                                                           //TODO Ver: IF  PAGE.RUNMODAL(0,lrDimVal) = ACTION::LookupOK THEN
                                                           //TODO Ver:   "Codigo MdM" := lrDimVal.Code;
        END;
    end;
            ...
            349:
                BEGIN // Dimensiones
                    CASE prImpTabl.Tipo OF
                        0:
                            pwTipo := "Tipo Registro"::"Serie/M todo";
                        1:
