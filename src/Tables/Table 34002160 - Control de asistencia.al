table 34002160 "Control de asistencia"
{
    ...
    begin
        ...
        */
        ...
        IF ("1ra entrada" >= 180000T) AND (NOT Festivo) THEN BEGIN
            IF iHora >= 8 THEN BEGIN
                txtHora := FORMAT(8);
        ...
        IF (iHora > 0) OR (iMin > 0) THEN BEGIN
            IF NOT Festivo THEN BEGIN
                IF iHora >= 3 THEN BEGIN
                    txtHora := FORMAT(3);
        ...
        IF ((iHora > 0) OR (iMin > 0)) AND (NOT Festivo) THEN BEGIN
            txtHora := FORMAT(iHora) + '.' + FORMAT(iMin);
            EVALUATE("Horas extras 100", txtHora);
            iHora := 0;
        END;

        IF ((iHora > 0) OR (iMin > 0)) AND (Festivo) THEN BEGIN
            txtHora := FORMAT(iHora) + '.' + FORMAT(iMin);
