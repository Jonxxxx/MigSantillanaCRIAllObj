table 75004 "Imp.MdM Tabla"
{
    ...
    begin
        ...
        ELSE BEGIN  // Estos son valores "Virtuales"
          CASE "Id Tabla" OF
            27 : BEGIN // Productos
                   CASE pwFieldNo OF
                     -101 : FieldName := Text0002;  // Alto
                     -102 : FieldName := Text0003;  // Ancho
                     -103 : FieldName := Text0004;  // Peso
                     // Dimensiones
                     -299..-200 : FieldName := Text0001 + cFuncMdM.GetDimCode(ABS("Id Tabla"+200), FALSE);
                   END;
                 END;
          END;
        END;
    ...
    begin
        ...
        CASE Visible OF
          Visible::S  : Value := FORMAT(TRUE);
          Visible::No : Value := FORMAT(FALSE);
        END;
    end;
    ...
    begin
        ...
        CASE Visible OF
          Visible::S  : Value := FORMAT(FALSE);
          Visible::No : Value := FORMAT(TRUE);
        END;
    end;
        ...
        FOR lwN := 1 TO lrPK.FIELDCOUNT DO BEGIN
          ...
          ELSE BEGIN
            CASE lwField2.NUMBER OF
              GetIdTipoField : cGenProd.SetFieldValue(lwField2, GetTipoText, FALSE); // Por defecto No validamos la clave primaria
              GetIdCodeField : cGenProd.SetFieldValue(lwField2, Code, FALSE);
              GetIdDescField : cGenProd.SetFieldValue(lwField2, Descripcion, FALSE);
            END;
          END;
        END;
